...
Epigenomics Project, restricted to a short region on chromosome 19. In the code below, we import the input to chromswitch and run chromswitch on our dataset,
...
interest
2. **Metadata dataframe**: a dataframe with at least two columns: *Sample* which stores
...
3. **Peaks or epigenomic features**: a list of `GRanges` objects, each of which stores
peaks or features for one sample, with elements named according to the sample IDs as
specified in the metadata

Each of these inputs can be imported from TSV or BED files, as shown below. Learn more about `GRanges` objects by checking out `r Biocpkg("GenomicRanges")`.
...
# Import BED files containing MACS2 narrow peak calls in using 
...
To import BED files containing peak MACS2 narow peak calls, we can use a helper
...
To read in features in other formats where the first three columns are *chr*, 
...
Alternatively, if your epigenomic features are stored in another tabular format,
