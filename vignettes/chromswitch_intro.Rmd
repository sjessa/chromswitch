...
1. Genomic query region: a GRanges object
2. Metadata dataframe
3. Peaks/epigenomic features
```{r quickstart}

# Define query regions
query <- GRanges(seqnames = c("chr19", "chr19"),
                                  ranges = IRanges(
                                      start = c(54924104,54874318),
                                      end = c(54929104, 54877536)))
# Prepare metadata
samples    <- c("E068", "E071", "E074", "E101", "E102", "E110")
infiles   <- system.file("extdata", paste0(samples, ".H3K4me3.bed"),
                        package = "chromswitch")
...
```{r quickstart_1}
callSummary(query = query,     # Input 1: Query region
...
Chromswitch outputs the Average_Silhouette (a measure of cluster quality), the score predicting a chromatin state switch (Consensus), and the cluster assignment for each sample.
...
1. One or more genomic regions which will serve as query regions (note that
chromswitch acts at the level of individual regions and so will return results for each region independently)
2. Metadata which describes the samples and their conditions
...
switches in three genes on chromosome 19. The `makeGRangesFromDataFrame` function from `r Biocpkg("GenomicRanges")` is
